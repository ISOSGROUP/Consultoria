...
$.fn.fab = function (options) {
  ...
  $('#fab').attr('onclick', 'check_permissions()');
...
/*
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      (( res[0].upload_files == 1) ? $("#upload").show() : $("#upload").hide() )
    }
  });
...
function toggleSelected (e) {
 ...
 // var sessName = '<?php echo $_SESSION['name']?>';
    //var upload_files = '<?php echo Config::get("permissionlist.upload_files"); ?>';
    //var download_files = "<?php echo Config::get('permissionlist.download_files'); ?>";
   // var create_folders = "<?php echo Config::get('permissionlist.create_folders'); ?>";

    //var r = "";
   //alert(sessName);
    //(( upload_files == 1) ? $("[data-action=download]").show() : $("[data-action=download]").hide() )
    //(( upload_files == 1) ? $("[data-action=trash]").show() : $("[data-action=trash]").hide() )
...
/*
    if(folder_name == 1){
      ...
      $.when(query()).done(function(res){
        ...
        (( res[0].download_files == 1) ? $("[data-action=download]").show() : $("[data-action=download]").hide() )
...
function performLfmRequest(url, parameter, type) {
 ...
 // alert("url: "+ url+" parameter: "+JSON.stringify(parameter)+" type: "+type);
